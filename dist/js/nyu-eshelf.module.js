...
.factory('nyuEshelfConfigService', ['nyuEshelfConfigDefaults', 'nyuEshelfConfig', '$location', function (defaults, config, $location) {
  ...
  var mergedConfig = angular.merge(defaults, config);
...
.factory('nyuEshelfService', ['nyuEshelfConfigService', '$http', function (config, $http) {
  return {
    ...
    initEshelf: function initEshelf() {
      var svc = this; // For maintaining service scoping in the function below
      // Eshelf API to setup csrfToken and avoid that pesky cache
      var url = config.envConfig.eshelfBaseUrl + "/records/from/primo.json?per=all&_=" + Date.now();
    ...
    checkEshelf: function checkEshelf(externalId) {
      // Use eshelf API to check if record with externalId is in current user's eshelf
      var url = config.envConfig.eshelfBaseUrl + "/records/from/primo.json?per=all&external_id[]=" + externalId;
      var svc = this; // For maintaining service scoping in the function below
      $http.get(url).then(function (response) {
    ...
    generateRequest: function generateRequest(httpMethod, data) {
      // Whitelist http methods DELETE and POST
      if (!/^(DELETE|POST)$/.test(httpMethod.toUpperCase())) {
        return {};
      }
      // Cors headers
      var headers = { 'X-CSRF-Token': this.csrfToken, 'Content-type': 'application/json;charset=utf-8' };
      var request = {
    ...
    failure: function failure(response, externalId) {
      this[externalId + '_error'] = true;
    },
    // Set the new csrfToken to the response header on success
    success: function success(response, externalId) {
      if (response.headers('x-csrf-token')) {
...
.controller('nyuEshelfController', ['nyuEshelfService', 'nyuEshelfConfigService', '$rootScope', '$scope', '$http', '$location', '$window', function (nyuEshelfService, config, $rootScope, $scope, $http, $location, $window) {
  ...
  $scope.disabled = function () {
    return nyuEshelfService[$scope.externalId + '_error'] || $scope.running;
  };
  // In eshelf?
  ...
  $scope.addToEshelf = function () {
    $scope.toggleInEshelf('post');
  };
  // Alias to delete
  $scope.removeFromEshelf = function () {
    $scope.toggleInEshelf('delete');
  };
  // Wrap the generic request
  $scope.toggleInEshelf = function (httpMethod) {
    $http(nyuEshelfService.generateRequest(httpMethod, $scope.recordData)).then(function (response) {
      $scope.running = false;nyuEshelfService.success(response, $scope.externalId);
    }, function (response) {
...
.component('nyuEshelf', {
  ...
  template: '<div class="nyu-eshelf neutralized-button md-button md-primoExplore-theme">' + '<input ng-checked="inEshelf()" aria-label="Toggle in e-Shelf" ng-disabled="disabled()" id="{{ elementId }}" type="checkbox" data-eshelf-external-id="{{ externalId }}" ng-click="running = true; eshelfCheckBoxTrigger()" >' + '<label for="{{ elementId }}"><span ng-bind-html="setElementText()"></span></label>' + '</div>'
})
...
.component('nyuEshelfToolbar', {
  ...
  template: '<md-button class="button-with-icon zero-margin md-button md-primoExplore-theme md-ink-ripple {{myEshelfButtonClasses}}" type="button" aria-label="Go to {{ elementText }}" ng-click="openEshelf()">' + '<md-tooltip md-direction="bottom" md-delay="500">Go to {{ elementText }}</md-tooltip><prm-icon style="z-index:1" icon-type="svg" svg-icon-set="image" icon-definition="ic_collections_bookmark_24px" aria-label="Go to {{elementText }}"></prm-icon>' + '<span class="hide-xs">{{ elementText }}</span>' + '</md-button>'
});
//# sourceMappingURL=nyu-eshelf.module.js.map