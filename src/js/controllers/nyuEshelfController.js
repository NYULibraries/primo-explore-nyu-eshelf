nyuEshelfController.$inject = [
  "nyuEshelfService",
  "nyuEshelfConfigService",
  "$scope",
  "$http",
  "$location",
  "$window",
];
function nyuEshelfController(
  nyuEshelfService,
  config,
  $scope,
  $http,
  ...
  this.$onInit = () => {
    // Primo ID
    $scope.externalId =
      ctrl.prmSearchResultAvailabilityLineCtrl.result.pnx.control.recordid[0];
    $scope.elementId =
      "eshelf_" +
      $scope.externalId +
    ...
    nyuEshelfService.loggedIn =
      !ctrl.primoExploreCtrl.userSessionManagerService.isGuest();
    // Build the pds url
    $scope.pdsUrl =
      config.envConfig.pdsUrl.base +
      "?func=load-login&calling_system=" +
      config.envConfig.pdsUrl.callingSystem +
      "&institute=" +
      config.envConfig.institution +
      "&url=" +
      config.primoBaseUrl +
      "/primo_library/libweb/pdsLogin?targetURL=" +
      $window.encodeURIComponent($location.absUrl()) +
      "&from-new-ui=1&authenticationProfile=BASE_PROFILE";

    const inGuestText =
      config.inGuestEshelf +
      (config.loginToSave && config.loginToSave != ""
        ? ' (<a href="' + $scope.pdsUrl + '">' + config.loginToSave + "</a>)"
        : "");

    $scope.inEshelfText = nyuEshelfService.loggedIn
      ? config.inEshelf
      : inGuestText;
  };
  ...
  $scope.disabled = () =>
    Boolean(
      !nyuEshelfService.initialized ||
        nyuEshelfService[$scope.externalId + "_error"] ||
        $scope.running
    );
  // In eshelf?
  ...
  $scope.toggleInEshelf = (httpMethod) => {
    const request = nyuEshelfService.generateRequest(
      httpMethod,
      $scope.recordData
    );
    $http(request).then(
